...
class ProductController extends Controller
{
    ...
    public function edit(string $slug)
    {
        ...
        $Category = MainCategory::with('subCategories')->latest()->get();
        $size = Size::latest()->get();
        return view('admin.product.edit_product', compact('product', 'Category', 'size'));
    }
    ...
    //         if ($product) {
    //             return edirect('admin/products/view-product')->with('success', 'Product Edited Successfully!');
    //         }
    //     } catch (\Exception $e) {
    //         return back()->with('error', $e->getMessage());
    //     }
    ...
    public function update(ProductCreateRequest $request)
    {
        try {
            $product = DB::transaction(function () use ($request) {
                ...
                // Remove old size and price relationships
                ProductSizePrice::where('product_id', $product->id)->delete();

                // Add new size and price relationships
                if (!empty($size) && !empty($price) && !empty($product_stock) && count($size) === count($price) && count($price) === count($product_stock)) {
                    for ($i = 0; $i < count($size); $i++) {
                        if ($size[$i] !== null && $price[$i] !== null && $product_stock[$i] !== null && $size[$i] !== '' && $price[$i] !== '' && $product_stock[$i] !== '') {
                            $productSizePrice = new ProductSizePrice();
